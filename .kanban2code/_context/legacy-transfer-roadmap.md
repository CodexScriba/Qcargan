...
Rebuild the QueCargan platform in two phases:

**Phase 1 (This Roadmap):** Production landing page with authentication infrastructure, analytics, and deployment. Focus: get a secure, validated MVP to production quickly.

**Phase 2 (Separate Roadmap):** Vehicle marketplace features (listings, details, admin). Focus: build the directory once landing page is live and infrastructure is proven.

The rebuild uses a fresh tech stack (Next.js 16, Supabase, Drizzle, next-intl, Tailwind v4, shadcn/ui) with fresh credentials and improved code organization, prioritizing security and developer understanding after the react2shell vulnerability incident.
...
- **Vehicle marketplace features** — Moved to Phase 2 (separate roadmap)
...
> **Phase 2 will cover:** Vehicle listings page, vehicle detail page, product components (carousel, cards, specs), and admin dashboard.
...
### Public Pages
- [ ] Landing page (hero, value props, CTA/waitlist)
- [ ] Placeholder pages (services, shop, used cars waitlist)

### Product Components
> **Note:** Moved to Directory Roadmap (separate task)
- [ ] ProductTitle
- [ ] ImageCarousel
...
22. Landing page (hero, value props, CTA)
23. Placeholder pages (services, shop, used cars waitlist)

### Phase 7: Polish & Security
24. Complete translations (es/en for landing + auth pages)
25. Security audit (dependencies, RLS, input validation)

### Phase 8: Analytics & Deployment
26. PostHog integration (client + server)
27. Coolify deployment configuration
28. Production environment setup

### Phase 9: Directory (Separate Roadmap)
> **Note:** Vehicle marketplace features moved to separate roadmap
- [ ] Vehicles listing page (filters, pagination)
- [ ] Vehicle detail page (specs, pricing, financing, reviews)
- [ ] Product components (carousel, cards, specs, seller cards)
- [ ] Admin dashboard (protected CRUD operations)
...
**Objective:** Complete translations and perform security audit before production.

#### Task 7.1: Complete Translation Files
**Definition of Done:**
- [ ] All UI strings translated to Spanish and English (landing + auth pages)
...
**Objective:** Add analytics and deploy landing page to production.
...
- [ ] Key events defined (signup, login, waitlist_signup)
...
### Phase 9: Directory Roadmap (Separate)

**Objective:** Build vehicle marketplace features in a dedicated roadmap.

This phase is intentionally deferred and will be planned in a separate roadmap (`roadmap-directory.md`). It includes:

- **Vehicles Listing Page** - Grid with filters, pagination, server-side data fetching
- **Vehicle Detail Page** - Image carousel, specs, seller cards, financing calculator
- **Product Components** - ImageCarousel, VehicleCard, SellerCard, VehicleSpecs, FinancingTabs
- **Admin Dashboard** - Protected CRUD operations for vehicles and organizations

**Rationale:** Splitting the directory into a separate roadmap allows:
1. Clear separation between MVP (landing + auth) and features requiring marketplace data
2. Dedicated focus on component design and data modeling
3. Cleaner task tracking and dependencies
